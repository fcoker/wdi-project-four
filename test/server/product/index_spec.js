    ...
    api.get('/api/')
      .end((err, res) => {
        res.body.forEach(product => {
          ...
          expect(res.body.name).to.eq(dataItem.name);
          expect(res.body.format).to.eq(dataItem.format);
          expect(res.body.genre).to.eq(dataItem.genre);
          expect(res.body.images).to.eq(dataItem.images);
          expect(res.body.video).to.eq(dataItem.video);
          expect(res.body.description).to.eq(dataItem.description);
          expect(res.body.releaseDate).to.eq(dataItem.releaseDate);
        });
        done();
      });
  });
